...
// Find the index of "Save Message"
const marker = 'Save Message';
const idx = content.indexOf(marker);

if (idx !== -1) {
    // Find the next '</div>' three times.
    let currentIdx = idx;
    for(let i=0; i<3; i++) {
        currentIdx = content.indexOf('</div>', currentIdx + 1);
    }
    
    let cleanContent = content.substring(0, currentIdx + 6);
...
`;
    ...
    fs.writeFileSync('src/pages/SettingsPage.jsx', cleanContent + modals);
    console.log("Successfully cleaned and appended.");
} else {
