...
async function testRace() {
  ...
  results.forEach((res, i) => {
    if (res.status === 'fulfilled' && res.value.status === 201) {
      ...
      console.log(`Req ${i+1}: Failed, Status ${res.value.status}, Error: ${res.value.data.message}`);
    } else {
      console.log(`Req ${i+1}: Failed unexpectedly, Error: ${res.reason}`);
    }
  });
  
  const count = await Registration.countDocuments({ candidates: candId, programme: progId });
  console.log(`Final Database Registration Count: ${count}`);
