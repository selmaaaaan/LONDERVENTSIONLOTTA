...
walkDir('admin-hudafestival-main/src/pages', (filePath) => {
    if (filePath.endsWith('.jsx')) {
        ...
        if (/\balert\(/.test(c) || /window\.alert\(/.test(c)) files.push(filePath);
    }
});
...
files.forEach(f => {
    ...
    // Add hook to the main component
    const compRegex = /((?:export\s+(?:default\s+)?)?(?:function\s+[A-Z]\w*\s*\([^)]*\)\s*\{|const\s+[A-Z]\w*\s*=\s*(?:\([^)]*\))?\s*=>\s*\{))/;
